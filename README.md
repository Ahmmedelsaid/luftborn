...
**Live demo:** <https://ahmmedelsaid.github.io/luftborn/> — deployed from `main`
by CI. It runs with no server at all; see [The live demo](#the-live-demo) for
what that means.
...
| `npm run build:demo`         | Build the GitHub Pages demo (needs no server)          |
| `npm run generate:demo-data` | Snapshot the mock database for the demo build          |
...
<https://ahmmedelsaid.github.io/luftborn/>

GitHub Pages serves files and runs no processes, so json-server cannot live
there. Rather than publish a read-only demo, the `demo` build swaps one file:

```
angular.json -> configurations.demo.fileReplacements
  src/demo/demo.providers.ts  ->  src/demo/demo.providers.demo.ts
```

That adds `demoBackendInterceptor` as the **last** interceptor in the chain,
standing in for the network itself. Caching, retrying and error normalisation
all still run above it exactly as they do against the real API, so the demo
exercises the real code path — only the far end differs.

Reads come from a snapshot of `server/db.js`, generated by
`npm run generate:demo-data`. Running the real module rather than re-deriving
anything is what keeps the demo's `users` and `activities` identical to what
json-server serves, since both are derived from the tasks and exist nowhere on
disk.

**Everything works, writes included.** Creating, editing, deleting and
reordering all mutate the snapshot in the tab, so the board behaves as it does
locally.

Two consequences, neither a defect:

- **Data is per-visitor.** Two people see their own copies.
- **A reload restores the original dataset.** The snapshot is cloned at
  start-up rather than persisted.

Deep links work through a copy of `index.html` at `404.html`. Pages has no SPA
rewrite, but it does serve `404.html` for an unknown path, which boots the app
and lets the router take over. Clicking through the app issues no server request
at all; a pasted deep link or a refresh returns a 404 status carrying the
application, which is the standard Pages trade-off for keeping clean URLs.

The interceptor is compiled into this build and no other. `npm run build`
produces the same bundle it always did, and the demo data is not in it.
...
`.github/workflows/ci.yml`. Four parallel jobs on every push and pull request
to `main`, plus a deploy that runs only on `main` once they have all passed:
`main`:
...
| **Deploy demo to Pages**    | `main` only, and only once all four gates pass — publishes the demo                               |
